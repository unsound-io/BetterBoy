...
let vram () =
  ...
  let vram =
    Lwd.map (Lwd.get Common.machine) ~f:(fun m ->
    let go mode tiles =
  ...
  in
  let current =
    Lwd.map2 (Lwd.get current_tile) (Lwd.get Common.machine) ~f:(fun (tile, mode) m ->
    ...
    Ui.vcat [t; Ui.space 1 1; addr; Ui.space 1 0; num;])
  in
  Lwd_utils.pack Ui.pack_x [Lwd.join vram; Ui.space 1 1 |> Lwd.pure; current]
